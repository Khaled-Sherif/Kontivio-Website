      ...
      <main>
        ...
        <section className="k-section" style={{ background: 'white' }}>
          <div className="k-svc-grid k-reveal" style={{ maxWidth: 1200, margin: '0 auto' }}>
            {posts.map((p) => (
              <Link key={p.slug} to={`/blog/${p.slug}`} className="k-team-card" style={{ textDecoration: 'none', display: 'block' }}>
                <div className="k-team-card-img" style={{ height: 200 }}><img src={p.image} alt={p.title} loading="lazy" /></div>
